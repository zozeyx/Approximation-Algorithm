...
public class JobScheduling {
    ...
    public static int approximateJobScheduling(int m, int[] operation_time) {
        ...
        // 각 작업에 대해 처리
        for (int i = 0; i < n; i++) {
            int minMachine = 0; // 가장 빨리 종료되는 기계 변수
            ...
            L[minMachine] += operation_time[i]; // 종료 시간 갱신
        ...
        for (int i = m - 1; i >= 0; i--) { //
            ...
            String[] timeSlot = new String[13];
            Arrays.fill(timeSlot, " "); 

            // 기계에서 실행된 각 작업의 시작~종료 시간대에 배치
            ...
            for (int time = 0; time <= 12; time++) {
                System.out.printf("%3s", timeSlot[time]); 
            }
            System.out.println();
        }
